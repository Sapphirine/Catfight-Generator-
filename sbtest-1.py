...
subre_dict = dict((k,v) for k,v in names_count.items() if v >= 500)       #pick subreddit that shows at least 1000 times

authors = [i[2] for i in table]
authors_count = Counter(authors)
authors_dict = dict((k,v) for k,v in authors_count.items() if v >= 0)     #pick author who posts at least 10 times
...
word_list = dict((k,v) for k,v in total_words_count.items() if v > 10)          #pick words that show up more than 50 times
...
for x in range(1,len(filtered_table)):                                  #append words in each line to each list under each subreddit
...
for x in range(len(word_list2)):                                      #put word count result from each dictionary from each subreddit
    print x
    for y in range(1,len(subre_dict2)):
        if word_list2[x] in df[y]:
            final_matr[x][y] =  df[y][word_list2[x]]
